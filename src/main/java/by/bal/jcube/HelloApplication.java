...
public class HelloApplication extends Application {
    ...
    private void setupKeyboardControls(Scene scene) {
        scene.setOnKeyPressed(event -> {
            ...
            switch (event.getCode()) {
                case UP:
                    cameraPosition.y -= moveSpeed; // Движение вперёд (Z уменьшается)
                    break;
                case DOWN:
                    cameraPosition.y += moveSpeed; // Движение назад
                    break;
                case LEFT:
                    cameraPosition.x -= moveSpeed; // Движение влево
                    break;
                case RIGHT:
                    cameraPosition.x += moveSpeed; // Движение вправо
                    break;
                case W:
                    cameraPosition.z += moveSpeed; // Вверх (дополнительно)
                    break;
                case S:
                    cameraPosition.z -= moveSpeed; // Вниз
                    break;
            }
        });
    }
    ...
    private void initMeshes() throws IOException {
        ...
        //
        //
        // Vector3 offset = Vector3.of(-center.x, -center.y, -center.z + 1);
        //
        // center.add(offset);
        //
        // for (Vector3 vertex : vertices) {
